...
function update_total() {
    ...
    $('.sub-total').val(roundNumber(total, 2));
    $('.total-discount').val(roundNumber(discount, 2));
    $('.tax-total').val(roundNumber(taxprice, 2));
    $('.total-amount').val(roundNumber(amount, 2));
    $('.due-amount').val(roundNumber(due, 2));
}
...
function initAutocomplete() {
    $(".item-name").autocomplete({
        ...
        select: function( event, ui ) {
            ...
            ele.find(".item-name").val(ui.item.label.replace(/&amp;/g, '&'));
            ele.find(".item-descr").val(ui.item.description.replace(/&amp;/g, '&'));
